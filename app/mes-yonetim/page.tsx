  ...
  return (
    <div className="min-h-screen bg-gray-50/50 text-gray-900 overflow-x-hidden">
      ...
      <section className="relative pt-32 pb-20 md:pt-44 md:pb-28 overflow-hidden bg-white border-b border-gray-100">
        <div className="absolute inset-0 bg-[linear-gradient(to_bottom,rgba(238,242,255,0.4),rgba(255,255,255,0))] -z-10" />
        <div className="absolute top-1/4 right-10 w-96 h-96 bg-indigo-200/20 rounded-full blur-3xl -z-10" />
        
        <div className="max-w-7xl mx-auto px-4 sm:px-6 relative">
          <div className="text-center max-w-3xl mx-auto">
            <span className="inline-flex items-center gap-1.5 px-4 py-1.5 bg-indigo-50 border border-indigo-200/60 rounded-full text-indigo-700 text-xs sm:text-sm font-bold shadow-sm mb-6">
            ...
            <h1 className="text-4xl sm:text-5xl md:text-6xl font-black tracking-tight mb-6 leading-tight">
              ...
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-600 via-indigo-800 to-slate-900">
                Mikro Entegre MES
              </span>
            </h1>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-4xl mx-auto mt-12">
            {stats.map((stat, i) => (
              <div key={i} className="bg-white border border-gray-300/80 p-6 rounded-2xl shadow-md shadow-gray-100/50 hover:shadow-lg transition-shadow">
                <div className="text-3xl sm:text-4xl font-black text-indigo-600 mb-1">{stat.value}</div>
      ...
      <section className="py-16 sm:py-24 max-w-6xl mx-auto px-4 sm:px-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-stretch">
          ...
          <div className="lg:col-span-5 bg-gradient-to-br from-indigo-500/10 via-indigo-500/5 to-white border border-indigo-200 rounded-3xl p-6 sm:p-8 flex flex-col justify-between shadow-sm relative overflow-hidden">
            <div className="absolute -top-10 -right-10 w-40 h-40 bg-indigo-500/5 rounded-full blur-2xl" />
            <div>
              <div className="w-12 h-12 bg-indigo-600 text-white rounded-2xl flex items-center justify-center mb-6 shadow-md shadow-indigo-600/20">
            ...
            <div className="border-t border-indigo-100/60 pt-4 flex items-center gap-3">
              <div className="w-2.5 h-2.5 bg-indigo-600 rounded-full animate-pulse" />
              <span className="text-xs font-bold text-indigo-700 uppercase tracking-wider">Mikro ERP Entegrasyonlu</span>
            </div>
          </div>
          ...
          <div className="lg:col-span-7 space-y-4 flex flex-col justify-center">
            {[
              ...
              <div key={idx} className="bg-white border border-gray-300/80 rounded-2xl p-5 shadow-sm hover:shadow-md transition-shadow flex gap-4">
                <div className="w-6 h-6 bg-indigo-500/10 text-indigo-600 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5">
      ...
      <section className="bg-white border-y border-gray-100 py-16 sm:py-24">
        <div className="max-w-6xl mx-auto px-4 sm:px-6">
          <div className="bg-gradient-to-br from-indigo-500/5 to-violet-500/5 border border-indigo-200/40 rounded-3xl p-8 sm:p-12 flex flex-col lg:flex-row items-center gap-10">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-3">
                {[
                  ...
                  <div key={idx} className="flex items-center gap-2 text-xs sm:text-sm text-gray-700">
                    <Check className="w-4.5 h-4.5 text-indigo-600 flex-shrink-0" />
            ...
            <div className="lg:w-2/5 flex justify-center">
              <div className="relative w-full max-w-[280px] h-[280px] bg-white border border-gray-200 rounded-3xl shadow-xl flex items-center justify-center p-6">
                <div className="absolute inset-0 bg-gradient-to-r from-indigo-100 to-violet-100 blur-2xl opacity-40 -z-10" />
                <div className="text-center space-y-4">
                  <div className="w-16 h-16 bg-gradient-to-tr from-indigo-600 to-indigo-800 text-white rounded-2xl flex items-center justify-center mx-auto shadow-lg shadow-indigo-600/20">
      ...
      <section className="py-16 sm:py-24 max-w-6xl mx-auto px-4 sm:px-6">
        <div className="text-center mb-12">
          ...
          {/* Custom Tabs Toggle */}
          <div className="inline-flex p-1.5 bg-indigo-700 rounded-2xl border border-indigo-800/50 mt-4 shadow-lg">
            <button
              onClick={() => setActiveTab('features')}
              className={`px-6 py-2.5 rounded-xl text-xs sm:text-sm font-bold transition-all duration-200 ${activeTab === 'features' ? 'bg-white text-indigo-700 shadow-sm' : 'text-indigo-100 hover:text-white'}`}
            >
              Ana Özellikler
            </button>
            <button
              onClick={() => setActiveTab('usage')}
              className={`px-6 py-2.5 rounded-xl text-xs sm:text-sm font-bold transition-all duration-200 ${activeTab === 'usage' ? 'bg-white text-indigo-700 shadow-sm' : 'text-indigo-100 hover:text-white'}`}
            >
              Nasıl Çalışır?
            </button>
          </div>
        ...
        <AnimatePresence mode="wait">
          {activeTab === 'features' ? (
              ...
              {features.map((feat, i) => {
                ...
                return (
                  <div key={i} className="bg-white border border-gray-300/80 p-6 rounded-2xl shadow-sm hover:shadow-md transition-shadow group">
                    <div className="w-10 h-10 bg-indigo-500/10 text-indigo-600 rounded-xl flex items-center justify-center mb-4 group-hover:bg-indigo-600 group-hover:text-white transition-colors duration-300">
                      <Icon size={18} className="text-indigo-600 group-hover:text-white transition-colors duration-300" />
                    </div>
            ...
            <motion.div
              ...
              exit={{ opacity: 0, y: -15 }}
              transition={{ duration: 0.3 }}
              className="relative border-l border-indigo-200/60 ml-4 md:ml-10 space-y-12"
            >
              {usageSteps.map((step, i) => (
                <div key={i} className="relative pl-8 sm:pl-12">
                  <div className="absolute -left-[17px] top-0 w-8 h-8 rounded-full bg-gradient-to-br from-indigo-600 to-indigo-800 text-white flex items-center justify-center font-black text-xs shadow-md shadow-indigo-600/15">
                    {step.step}
                  </div>
            ...
            {sss.map((f, i) => (
              <details key={i} className="group rounded-2xl border border-gray-200 bg-white overflow-hidden">
                <summary className="flex items-start gap-4 p-5 cursor-pointer list-none font-semibold text-gray-900">
                  <span className="w-8 h-8 bg-indigo-600 rounded-full flex items-center justify-center text-white text-sm font-bold flex-shrink-0">{i + 1}</span>
      ...
      <section className="py-16 sm:py-24 max-w-5xl mx-auto px-4 sm:px-6">
        <div className="relative rounded-3xl overflow-hidden bg-gradient-to-br from-indigo-700 via-indigo-800 to-slate-900 text-white shadow-xl shadow-indigo-700/20 p-8 sm:p-12 md:p-16 text-center">
