...
export default function SectoralSolutionsDetailPage() {
  ...
  // Handle Hash Navigation and Session Storage on load, hash change, and popstate
  useEffect(() => {
    ...
    const updateTabFromEnvironment = () => {
      ...
      const tabParam = searchParams.get('tab') || searchParams.get('sector');
      const savedTab = typeof window !== 'undefined' ? sessionStorage.getItem('sectoral_active_tab') : null;

      if (validTabs.includes(hash)) {
        setActiveTab(hash);
        sessionStorage.setItem('sectoral_active_tab', hash);
      } else if (tabParam && validTabs.includes(tabParam)) {
        setActiveTab(tabParam);
        sessionStorage.setItem('sectoral_active_tab', tabParam);
  ...
  const handleTabChange = (tabId: string) => {
    ...
    if (typeof window !== 'undefined') {
      sessionStorage.setItem('sectoral_active_tab', tabId);
